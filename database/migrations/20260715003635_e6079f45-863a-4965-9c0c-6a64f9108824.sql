...
BEGIN
  ...
  END IF;
  IF _phone IS NULL OR _phone !~ '^[0-9]{10}$' THEN
...
REVOKE ALL ON FUNCTION public.register_participant(text, text, text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.register_participant(text, text, text, text, text) TO anon, authenticated, service_role;
